...
feature 'Bank Accounts Index', js: true do
  ...
  context 'with bank accounts' do
    ...
    scenario 'shows each bank account' do
      expect(all('.bank_account').count).to eq(@account_count)
    ...
    scenario 'deletes a bank account if confirmed' do
      accept_confirm do
        all('.delete_account').first.click
      end

      expect(page).to have_selector('.bank_account', count: @account_count - 1)
    end
    ...
    scenario 'does not delete a bank account if not confirmed' do
      ...
      expect(page).to have_selector('.bank_account', count: @account_count)
    end

    scenario 'link to the show path is on the page' do
      expect(page).to have_selector('.show_account', count: @account_count)
    end
  end
  ...
  context 'without bank accounts' do
    ...
    scenario 'correct no bank accout message' do
    ...
    scenario 'link to create new bank account exists' do
